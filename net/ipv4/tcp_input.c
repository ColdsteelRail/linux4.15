			 ...
			 const struct tcphdr *th)
{
	...
	if (sk->sk_srt && (skb->retrans || skb->updsack))
	{
		...
		{
			/* mark as lost */
			struct sk_buff *skb_to_mark;
			skb_to_mark = tcp_sacktag_skip(skb, sk, NULL, TCP_SKB_CB(skb)->ack_seq);	/* get the packet*/
			if (skb_to_mark)
				tcp_skb_mark_lost(tp, skb_to_mark);
			if(sk->sk_logme)
				printk(KERN_DEBUG "tankdcn: tcp_rcv_established: Try to retransmit a packet\n");
		}
